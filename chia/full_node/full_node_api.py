...
from dogia.consensus.block_creation import create_unfinished_block
from dogia.consensus.block_record import BlockRecord
from dogia.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from dogia.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from dogia.full_node.full_node import FullNode
from dogia.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from dogia.full_node.signage_point import SignagePoint
from dogia.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from dogia.protocols.full_node_protocol import RejectBlock, RejectBlocks
from dogia.protocols.protocol_message_types import ProtocolMessageTypes
from dogia.protocols.wallet_protocol import PuzzleSolutionResponse, RejectHeaderBlocks, RejectHeaderRequest
from dogia.server.outbound_message import Message, make_msg
...
from dogia.types.mempool_item import MempoolItem
from dogia.types.peer_info import PeerInfo
from dogia.types.unfinished_block import UnfinishedBlock
from dogia.util.api_decorators import api_request, peer_required, bytes_required, execute_task
from dogia.util.generator_tools import get_block_header
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSDogiaConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSDogiaConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSDogiaConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSDogiaConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    @peer_required
    @api_request
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSDogiaConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSDogiaConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSDogiaConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
