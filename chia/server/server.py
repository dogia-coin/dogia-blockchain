...
from dogia.server.introducer_peers import IntroducerPeers
from dogia.server.outbound_message import Message, NodeType
from dogia.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from dogia.util.errors import Err, ProtocolError
from dogia.util.ints import uint16
from dogia.util.network import is_localhost, is_in_network
...
class DogiaServer:
    def __init__(
        ...
        dogia_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSDogiaConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSDogiaConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.dogia_ca_crt_path, self.dogia_ca_key_path = dogia_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSDogiaConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.dogia_ca_crt_path, self.dogia_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSDogiaConnection] = None
        try:
            connection = WSDogiaConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSDogiaConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSDogiaConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSDogiaConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSDogiaConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSDogiaConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSDogiaConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSDogiaConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
    ...
    def is_trusted_peer(self, peer: WSDogiaConnection, trusted_peers: Dict) -> bool:
