...
from dogia.pools.pool_config import PoolWalletConfig, load_pool_config
...
from dogia.util.bech32m import decode_puzzle_hash
from dogia.util.config import load_config, save_config, config_path_for_filename
...
from dogia.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    def __init__(
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'dogia keys generate' or open the UI."
    ...
    async def get_cached_harvesters(self, connection: WSDogiaConnection) -> HarvesterCacheEntry:
