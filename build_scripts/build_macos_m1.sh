...
electron-packager . Dogia --asar.unpack="**/daemon/**" --platform=darwin \
...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Dogia-darwin-arm64/Dogia.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=dogiablockchain.provisionprofile \
...
mv Dogia-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Dogia-$DOGIA_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Dogia-darwin-arm64/Dogia.app Dogia-$DOGIA_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.dogia.blockchain \
...
# xcrun altool --notarize-app -f Dogia-0.1.X.dmg --primary-bundle-id net.dogia.blockchain -u username -p password
...
# xcrun stapler staple Dogia-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Dogia-0.1.X.dmg
