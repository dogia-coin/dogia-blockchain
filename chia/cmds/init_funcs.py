...
from dogia.consensus.coinbase import create_puzzlehash_for_pk
from dogia.ssl.create_ssl import generate_ca_signed_cert, get_dogia_ca_crt_key, make_ca_cert
from dogia.util.bech32m import encode_puzzle_hash
from dogia.util.config import (
    create_default_dogia_config,
    initial_config_file,
    load_config,
    save_config,
...
from dogia.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'dogia keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    dogia_ca_crt, dogia_ca_key = get_dogia_ca_crt_key()
    dogia_ca_crt_path = ca_dir / "dogia_ca.crt"
    dogia_ca_key_path = ca_dir / "dogia_ca.key"
    dogia_ca_crt_path.write_bytes(dogia_ca_crt)
    dogia_ca_key_path.write_bytes(dogia_ca_key)
    ...
    dogia_ca_crt, dogia_ca_key = get_dogia_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, dogia_ca_crt, dogia_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if dogia_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return dogia_init(root_path)


def dogia_version_number() -> Tuple[str, str, str, str]:
...
def dogia_minor_release_number():
    res = int(dogia_version_number()[2])
    print(f"Install release number: {res}")
    return res


def dogia_full_version_str() -> str:
    major, minor, patch, dev = dogia_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def dogia_init(root_path: Path):
    if os.environ.get("DOGIA_ROOT", None) is not None:
        print(
            f"warning, your DOGIA_ROOT is set to {os.environ['DOGIA_ROOT']}. "
            f"Please unset the environment variable and run dogia init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Dogia directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if DOGIA_ROOT is set, or if user has run dogia init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'dogia keys show --show-mnemonic-seed'")
