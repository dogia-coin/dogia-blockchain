...
from dogia.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from dogia.protocols import full_node_protocol
from dogia.protocols.full_node_protocol import RespondBlock
from dogia.rpc.rpc_server import start_rpc_server
from dogia.rpc.wallet_rpc_api import WalletRpcApi
from dogia.rpc.wallet_rpc_client import WalletRpcClient
from dogia.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestPoolWalletRpc:
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_dogia():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_dogia)
