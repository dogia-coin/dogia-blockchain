...
if [ "$(id -u)" = 0 ]; then
  echo "The Dogia Blockchain GUI can not be installed or run by the root user."
	exit 1
fi

# Allows overriding the branch or commit to build in dogia-blockchain-gui
SUBMODULE_BRANCH=$1
...
echo ""
echo "Dogia blockchain install-gui.sh completed."
echo ""
echo "Type 'cd dogia-blockchain-gui' and then 'npm run electron &' to start the GUI."
