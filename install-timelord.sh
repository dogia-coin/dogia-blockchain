...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the dogia python virtual environment."
...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("dogiavdf").location)' 2>/dev/null)/vdf_client
DOGIAVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("dogiavdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing dogiavdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary dogiavdf "$DOGIAVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary dogiavdf "$DOGIAVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing dogiavdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing dogiavdf requirement boost for MacOS."
		brew install boost
		echo "Installing dogiavdf from source."
