...
from dogia.protocols import full_node_protocol
from dogia.protocols.protocol_message_types import ProtocolMessageTypes
from dogia.protocols.shared_protocol import Handshake
from dogia.server.outbound_message import make_msg, Message
from dogia.server.rate_limits import RateLimiter
from dogia.server.server import ssl_context_for_client
from dogia.server.ws_connection import WSDogiaConnection
from dogia.types.peer_info import PeerInfo
from dogia.util.ints import uint16, uint64
...
class TestDos:
    ...
    async def test_large_message_disconnect_and_ban(self, setup_two_nodes):
        ...
        ssl_context = ssl_context_for_client(
            server_2.dogia_ca_crt_path, server_2.dogia_ca_key_path, server_2.p2p_crt_path, server_2.p2p_key_path
        )
    ...
    async def test_spam_tx(self, setup_two_nodes):
        ...
        ws_con: WSDogiaConnection = list(server_1.all_connections.values())[0]
        ws_con_2: WSDogiaConnection = list(server_2.all_connections.values())[0]
