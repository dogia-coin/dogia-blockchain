...
import dogia.server.ws_connection as ws  # lgtm [py/import-and-import-from]
...
from dogia.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from dogia.consensus.pot_iterations import calculate_sp_iters
from dogia.full_node.block_store import BlockStore
from dogia.full_node.bundle_tools import detect_potential_template_generator
from dogia.full_node.coin_store import CoinStore
from dogia.full_node.full_node_store import FullNodeStore
from dogia.full_node.mempool_manager import MempoolManager
from dogia.full_node.signage_point import SignagePoint
from dogia.full_node.sync_store import SyncStore
from dogia.full_node.weight_proof import WeightProofHandler
from dogia.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from dogia.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from dogia.server.node_discovery import FullNodePeers
...
from dogia.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from dogia.util.errors import ConsensusError, Err
from dogia.util.ints import uint8, uint32, uint64, uint128
from dogia.util.path import mkdir, path_from_root
from dogia.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
    ...
    def set_server(self, server: DogiaServer):
        ...
        elif self.config["port"] == 8886:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.dogia.net")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSDogiaConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSDogiaConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSDogiaConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSDogiaConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSDogiaConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSDogiaConnection):
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSDogiaConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSDogiaConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSDogiaConnection],
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSDogiaConnection] = None
    ) -> Optional[Message]:
