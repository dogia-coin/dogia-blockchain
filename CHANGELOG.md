...
## 1.2.0 Dogia blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `dogia plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Dogia-Network/dogia-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Dogia-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `dogia farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `dogia configure` now supports command line updates to peer count and target peer count.
...
- All Dogia repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `dogia plotnft` to manage pools.
...
- We updated dogiapos to version 1.0.3. This adds parallel reads to GetFullProof. Thanks to @marcoabreu ! We now print target/final directory early in the logs refs and log process ID. Thanks to @grayfallstown ! We are now using Gulrak 1.5.6.
@683280 optimized code in phase1.hpp. @jespino and @mrhacky started migrating to flags instead of booleans parameters for `show_progress` and `nobitfield`. If you are providing third-party tools you may need to make adjustments if relying on the dogiapos log.
- Updated dogiavdf to version 1.0.2 to fix certain tests.
...
- We have increased the strictness of validating Dogialisp in the mempool and clvm.
...
- Correctly display private key in `dogia keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `dogia keys show`.
...
- Thank you to @DogiaMineJP for various improvements.
...
- `dogia farm summary` better handles wallet errors.
...
### 1.1.7 Dogia Blockchain 2021-06-05
...
## 1.1.6 Dogia Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /dogia/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `dogia configure -t true|false`).
...
- We now use version 1.0.2 of dogiapos. We now reopen files on read or write failure with thanks to @mreid-moz! We also prevent dogiapos prover crashing for more bad plots.
...
## 1.1.5 Dogia Blockchain 2021-05-09
...
## 1.1.4 Dogia Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `dogia keys show`. Use `dogia keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Dogia Blockchain 2021-05-01
...
- We changed dogialisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- Dogialisp programs now remained serialized in Node for better performance.
...
## 1.1.2 Dogia Blockchain 2021-04-24
...
## 1.1.1 Dogia Blockchain 2021-04-21
...
## 1.1.0 Dogia Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Dogia Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of Dogialisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [dogialisp.com](https://dogialisp.com) shortly.
...
## 1.0.5 Dogia Blockchain 2021-04-14
...
## 1.0.4 Dogia Blockchain 2021-04-12
...
- The `src` directory in dogia-blockchain has been changed to `dogia` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `dogia/`
...
## 1.0.3 Dogia Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Dogia-Network/dogia-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `dogia keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of dogiapos 1.0.0 as well.

## 1.0.2 Dogia Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [dogiapos](https://github.com/Dogia-Network/dogiapos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/dogia-blockchain).
...
- @jespino changed `dogia keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `dogia keys show`.
...
## 1.0.1 Dogia Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `dogia keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/dogia-blockchain/).
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/dogia-blockchain).
...
- Mainnet uses port 8886 and other constants and service names were changed for mainnet.
...
- Migration with DOGIA_ROOT set does not crash dogia init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.dogia.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `dogia farm summary` will now display TXCH or XCH as appropriate.
...
- `dogia farm summary` displayed some incorrect amounts.
...
- `dogia netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `dogia netspace`. Thanks @altendky.
- There was a typo in `dogia plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/Dogia-Network/dogia-blockchain/issues/1189)
...
- Remove `dogia plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `dogia keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of dogiavdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from dogiavdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- We now hand build Apple Silicon native binary wheels for all dogia-blockchain dependencies and host them at [https://pypi.dogia.net/simple](https://pypi.dogia.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a dogialisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during DogiaLisp native deserialization.
...
- We have moved to a new and much higher scalability download.dogia.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.dogia.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.dogia.net/latest/Setup-MacOS.dmg).
- Dogia wheels not on Pypi and some dependecies not found there also are now on pypi.dogia.net.
...
- PRs and commits to the dogia-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `dogia farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `dogia wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.dogia.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `dogia farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/dogia-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `dogia wallet` now takes get_address to get a new wallet receive address from the CLI.
- `dogia plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- Dogialisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of dogia-blockchain, etc., instead of in blspy.
...
- In dogiavdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new dogiavdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in dogiavdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for dogiavdf.
- dogiabip158 was set to a gold 1.0 version.
- Comments to Dogialisp and clvm source have been updated for all of the Dogialisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in dogia-blockchain.
- aiohttp was updated to 3.7.4 to address a low severity [security issue](https://github.com/advisories/GHSA-v6wp-4m6f-gcjg).
- calccrypto/uint128_t was updated in the Windows dogiapos implementation. Dogiapos required some changes its build process to support MacOS ARM64.
...
- Adding Click changed the behavior of `dogia keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.dogia.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the dogia sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Dogia dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `dogia -upnp f` was added to disable uPnP.
- `dogia plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major dogialisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom dogialisp programs. You need to make a SpendBundle, which includes the puzzle reveal (dogialisp), a solution (dogialisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/dogia-blockchain).
- You can obtain a new wallet receive address on the command line with `dogia wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All dogialisp opcodes have been renumbered. This should be the last major breaking change for dogialisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Dogia-Network/dogia-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `dogia -h` and `dogia SUBCOMMAND -h` can be your guide.
...
- `dogia plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `dogia netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `dogia init` only migrates release candidate directories. The versioned sub directories under `~/dogia` will be going away before mainnet.
...
- `dogia configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Dogia-Network/dogia-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and dogialisp are in release form. We have one major change to dogialisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern DogiaSetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'dogia wallet get_transactions' in the command line to see your transactions.
- 'dogia wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Dogia Blockchain GUI](https://crowdin.com/project/dogia-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Dogia-Network/dogia-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Dogia-Network/dogia-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `dogia plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'dogia wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- We updated dogiapos to hopefully address some harvester crashes when moving plot files.
...
- You can now send and receive transactions with the command line. Try `dogia wallet -h` to learn more. Also, `dogia wallet` now requires a third argument of `show`, therefor you will use `dogia wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [dogia blockchain gui](https://crowdin.com/project/dogia-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `dogia init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `dogia init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
- We build both MacOS x86_64 and MacOS universal wheels for dogiapos, dogiavdf, blpsy, and dogiabip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- The GUI is now in a separate submodule repository from dogia-blockchain, [dogia-blockchain-gui](https://github.com/Dogia-Network/dogia-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `dogia-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `dogia wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- `dogia plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.

### Changed

- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 TXCH as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/Dogia-Network/clvm_rs) is complete.
- We have changed the way TLS between nodes and between dogia services work. Each node now has two certificate authorities. One is a public, shared CA that signs the TLS certificates that every node uses to connect to other nodes on 8886 or 58886. You now also have a self generated private CA that must sign e.g. farmer and harvester's certificates. To run a remote harvester you need a new harvester key that is then signed by your private CA. We know this is not easy for remote harvester in this release but will address it quickly.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Dogia-Network/dogia-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated Dogialisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Dogia-Network/dogia-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Dogia-Network/dogia-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, dogiapos, dogiavdf, and dogiabip158.
...
- `dogia plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and DogiaLisp code matches the CI compiled code.
- We have removed the '-r' flag from `dogia` as it was being overridden in most cases by the `-r` for restart flag to `dogia start`. Use `dogia --root-path` instead.
- `dogia -h` now recommends `dogia netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `dogia show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `dogia configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `dogia plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `dogia plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `dogia show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `dogia show` handles sub block 0 better.
...
- `dogia plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `dogia plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `dogia show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `dogia configure --enable-upnp`.
...
- `dogia configure` now lets you enable or disable uPnP.
...
- `dogia netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `dogia show` of the GUI use `dogia netspace -d 1000` as `dogia netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [dogia.net](https://dogia.net/). Among the improvements this gives the Dogia blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- You can now specify which private key to use for `dogia plots create`. After obtaining the fingerprint from `dogia keys show`, try `dogia plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
- We use a faster hash to prime function for dogiavdf from the current release of gmp-6.2.1 which we have upgraded dogiavdf and blspy to support.
- There is a new cli command - `dogia configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `dogia configure -h`. Note that if dogia services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Dogia application set to launch on start.
- Testnet is now running at the primary port of 58886. Update your routers appropriately. This opens 8886 for mainnet.
- All networking code has been refactored and mostly moved to websockets.
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.dogia/VERSION/config/`
...
- All appropriate Dogialisp smart transactions have been updated to use aggsig_me.
...
- dogiavdf now allows passing in input to a VDF for new consensus.
- sha256tree has been removed from Dogialisp.
- `dogia show -s` has been refactored to support the new consensus.
- `dogia netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.dogia/ directory to speed up total testing time.
- Changes were made to dogiapos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run dogia-blockchain.
...
- `dogia show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Dogia-Network/dogiapos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Dogia dependencies like dogiabip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Dogia-Network/dogia-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install dogia-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Dogia-Network/dogia-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Dogia-Network/dogia-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Dogia GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Dogia-Network/dogia-blockchain/tree/dev/electron-react/src/locales) of the dogia-blockchain repository.
- `dogia check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to dogiapos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [dogiapos release notes](https://github.com/Dogia-Network/dogiapos/releases/tag/0.12.32).
...
- dogiapos 0.12.32 fixed a an out of bounds read that could crash the plotter. It also contains a fix to better handle the case of drive letters on Windows.
...
- dogiavdf now has full IFMA optimizations for processors that support it.

### Changed

- Multithreading support in dogiapos, as well as a new algorithm which is faster and does 70% less IO. This is a significant improvement in speed, much lower total writing, and configurability for different hardware environments.
...
- The rpc interfaces of all dogia services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test dogia since Beta 12 should appear in your wallet.
...
- Dogiapos issue where memory was spiking and increasing
- Fixed working space estimates so they are exact
- Log all errors in dogiapos
...
- Long_description_content_type is now set to improve dogia-blockdogian's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Dogia Network at backup.dogia.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Dogia backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.dogia.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test dogia are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.dogia` directory.
...
- `dogia init` now automatically discovers previous releases in each new release.

### Fixed

- `dogia show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `dogia show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Dogia Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Dogia UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (dogia run farmer-only, wallet-only, timelord-only)
...
- `dogia show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `dogia show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Dogia simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Dogia implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test dogia between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test dogia from the earlier two releases however.
...
- The proof of space document in dogiapos has been updated to the new format and edited for clarity. Additionally GitHub actions now has the on demand ability to create the PDF version.
...
- `dogia show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Dogia)
- Various code review alerts for comparing to a wider type in dogiapos were fixed. Additionally, unused code was removed from dogiapos
...
- See wallet balances in command line: `dogia show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `dogia keys sign` and `dogia keys verify` to allow farmers to certify their ownership of keys.
...
- `dogia start wallet-server` changed to `dogia start wallet`, for consistency.
...
- Removed legacy scripts such as dogia-stop-server, dogia-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Dogia-Network/bls-signatures) to use the
...
- The various sub repositories (dogiapos, dogiavdf, etc.) now build ARM64 binary
...
- The dogia executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./dogia -h` from
`~\AppData\Local\Dogia-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Dogia.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.
...
"dogia plots create" and "dogia plots check".
- Add plot directories to config.yaml from the cli with "dogia plots add".
- Use real plot sizes in UI instead of a formula/
- HD keys now use EIP 2333 format instead of BIP32, for compatibility with
other chains.
- Keys are now derived with the EIP 2334 (m/12381/8886/a/b).
- Removed the ability to pass in sk_seed to plotting, to increase security.
- Linux builds of dogiavdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a Dogia Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.dogia/test-plots/`
- We have completely refactored all networking code towards making each Dogia service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make dogia start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Dogia Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting dogia now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'dogia keys' command line.
...
- We added a `dogia keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the dogia-blockchain, dogiapos, dogiavdf, dogiabip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `dogia netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- dogiapos is now easier to compile with MSVC.
...
- `dogia-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `dogia-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `dogia-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `dogia-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `dogia init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `dogia start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `dogia-start-harvester`.
...
- The user interface now refers to dogia coins with their correct currency code of XCH.
...
- Building the sub reposities (dogiapos, dogiavdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Dogia Proof of Space ([dogiapos](https://github.com/Dogia-Network/dogiapos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `dogia init` then sync on first start will happen automatically. "\$DOGIA_ROOT" users will need to delete `$DOGIA_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `dogia netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during dogia init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `dogia show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `dogia show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Dogia-Network/dogia-blockchain/wiki) and placed in [INSTALL](https://github.com/Dogia-Network/dogia-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Dogia-Network/dogia-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of dogia action commands. You can run `dogia version` or `dogia start node` for example. Just running `dogia` will show you more options. However `dogia-create-plots` continues to use the hyphenated form. Also it's now `dogia generate keys` as another example.
- Dogia start commands like `dogia start farmer` and `dogia stop node` now keep track of process IDs in a run/ directory in your configuration directory. `dogia stop` is unlikely to work on Windows native for now. If `dogia start -r node` doesn't work you can force the run/ directory to be reset with `dogia start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Dogia-Network/dogia-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `dogia start wallet-gui` has been chaned to `dogia start wallet` which launches but the GUI and server on MacOS and Linux. `dogia start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Dogia-Network/dogia-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `dogia stop all` command sometimes fails, use `dogia-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `dogia init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `dogia-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Dogia-Network/dogia-blockchain/wiki/Updating-beta-software). If you've set `$DOGIA_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `dogia show`.

### Changed

- `dogia-restart-harvester` has been renamed from `dogia-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.dogia/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Dogia blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Dogia-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the dogia management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `dogia-start-farmer &`. The [README.md](https://github.com/Dogia-Network/dogia-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.dogia/release/config, plots/ db/, wallet/ etc. This can be overridden by `export DOGIA_ROOT=~/.dogia` for example which would then put the plots directory in `HomeDirectory/.dogia/plots`.
- The libraries dogia-pos, dogia-fast-vdf, and dogia-bip-158 have been moved to their own repositories: [dogiapos](https://github.com/Dogia-Network/dogiapos), [dogiavdf](https://github.com/Dogia-Network/dogiavdf), and [chaibip158](https://github.com/Dogia-Network/dogiabip158). They are brought in by dogia-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Dogia-Network/bls-signatures).
- The install process now brings in dogiapos, dogiavdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
- `dogia-regenerate-keys` has been renamed `dogia-generate-keys`.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for dogiavdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.

### Removed

- The Beta release is not compatible with the history of the Alpha blockchain and we will be ceasing support of the Alpha chain approximately two weeks after the release of this Beta. However, your plots and keys are fully compatible with the Beta chain. Please save your plot keys! Examples of how to save your keys and upgrade to the Beta are available on the [repo wiki](https://github.com/Dogia-Network/dogia-blockchain/wiki).
- The ssh ui and web ui are removed in favor of the cli ui and the Electron GUI. To mimic the ssh ui try `dogia show -s -c` and try `dogia show --help` for usage instructions.
...
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/dogiavdf/fast_vdf/.
...
- Dogia fullnode, farmer and harvester now default to logging to dogia.log in the dogia-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Dogia blockchain.
...
- Added color logging
- Now dogia_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to dogia_full node, dogia_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new dogia services were started.
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 8886 or their UPnP port.
...
- This is the first release of the Dogia testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.dogia.net/developer/](https://www.dogia.net/developer/)

[unreleased]: https://github.com/Dogia-Network/dogia-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Dogia-Network/dogia-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Dogia-Network/dogia-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Dogia-Network/dogia-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Dogia-Network/dogia-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Dogia-Network/dogia-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Dogia-Network/dogia-blockchain/releases/tag/Alpha-1.0
